"""LGB run：holdout/submit 训练+评估+产出统一 metrics。

逻辑与 xgb/run.py 同构，仅模型名不同。评估原语走 eval_core（口径统一）。
@register_runner("lgb_ranker") 注册到 dispatcher。
"""
...
def run(cfg: dict) -> dict:
    ...
    model = get_model("lgb_ranker", model_params)
...
def _holdout(cfg, model, feat_cfg, topk, t0):
    print(f"[lgb] mode=holdout | 构建可信 holdout 特征...")
    ...
    print(f"[lgb] 训练模型 (fit={len(fit_groups)}组 watch={len(watch_groups)}组 early-stop)...")
    model.fit(fit_df, watch_df, feat_cols, cat_cols, fit_groups, watch_groups)
    print(f"      done ({time.time()-t0:.0f}s)")

    print(f"[lgb] val NDCG@{topk} (val 未参与训练/选模)...")
...
def _submit(cfg, model, feat_cfg, topk, t0):
    ...
    print(f"[lgb] 写出 {out_path}: {len(out)} 行 | 每行iid数 {lens.min()}~{lens.max()} "
